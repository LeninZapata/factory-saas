class ogTabs {
  ...
  static renderContentItem(item) {
    ...
    if (item.type === 'section') {
      const title = item.title ? `<h5 class="og-section-title">${ogI18n.processString(item.title)}</h5>` : '';
      const desc = item.description ? `<p class="og-section-description">${ogI18n.processString(item.description)}</p>` : '';
