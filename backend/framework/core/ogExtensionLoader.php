...
class ogExtensionLoader {
  ...
  static function load($extensionName, $router) {
    ...
    if (!file_exists($configFile)) {
      ogResponse::error(__('core.extension.not_found', ['extension' => $extensionName]), 404);
      return false;
    }
    ...
    if (!($config['enabled'] ?? false)) {
      ogResponse::error(__('core.extension.disabled', ['extension' => $extensionName]), 403);
      return false;
    }

    // Verificar que tiene backend
    if (!($config['backend'] ?? false)) {
      ogResponse::error(__('core.extension.no_backend', ['extension' => $extensionName]), 400);
      return false;
    }
