...
spl_autoload_register(function ($class) {
  ...
  // 4. Controllers - Framework primero, luego App
  $controllerFile = FRAMEWORK_PATH . '/controllers/' . $class . '.php';
  ...
  // 5. Handlers - Framework primero, luego App
  $handlerFile = FRAMEWORK_PATH . '/handlers/' . $class . '.php';
...
function handleClassNotFound($class) {
  ...
  ogLog::error(__('core.autoload.class_not_found', ['class' => $class]), [
    'class' => $class,
  ...
  if ($isApi || $isJson) {
    if (class_exists('response', false)) {
      ogResponse::error(__('core.autoload.class_not_found', ['class' => $class]), 500);
    } else {
