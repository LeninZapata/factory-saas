...
/**
 * docs-instructions.php
 * Genera:
 *   .github/docs-resumen-rapido.md  → contexto completo (se carga manualmente al inicio del chat)
 *   .github/copilot-instructions.md → solo reglas de estilo (se inyecta en cada request)
 * Llamado desde docs.php → docsInstructions($copilotPath)
 */

function docsInstructions(string $copilotPath): array {

  $generated = [];

  // ══════════════════════════════════════════════════════════════
  // 1. docs-resumen-rapido.md — contexto completo del framework
  //    Se manda manualmente con #file: solo al iniciar un chat
  //    de trabajo real. No se auto-inyecta.
  // ══════════════════════════════════════════════════════════════
  $initial = <<<'INITIAL_END'
...
INITIAL_END;
  ...
  file_put_contents($copilotPath . '/docs-resumen-rapido.md', $initial);
  $generated[] = '.github/docs-resumen-rapido.md';

  // ══════════════════════════════════════════════════════════════
  // 2. copilot-instructions.md — SOLO reglas de estilo
  //    Se auto-inyecta en CADA request de Copilot.
  //    Debe ser mínimo para no desperdiciar tokens.
  // ══════════════════════════════════════════════════════════════
  $style = <<<'STYLE_END'
...
- Nombres de archivos:
  ...
  - Schemas, rutas, helpers → camelCase: `saleStats.json`, `botWebhook.php`
...
STYLE_END;
  ...
  file_put_contents($copilotPath . '/copilot-instructions.md', $style);
